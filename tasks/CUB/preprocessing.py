...
if __name__ == '__main__':
    ...
    train_dataset = Cub2011(data_dir, train=True, download=True, transform=transform)
    test_dataset = Cub2011(data_dir, train=False, download=True, transform=transform)
    split = int(len(train_dataset) * 8 / 10)

    for dir_name in ['train', 'valid', 'test']:
        if not os.path.exists(f'{data_dir}/{dir_name}'):
            os.makedirs(f'{data_dir}/{dir_name}')

    for idx, (img, _) in enumerate(tqdm(train_dataset)): 
        if idx < split: dir_name = 'train'
        else: dir_name = 'valid'
        save_image(img, f'{data_dir}/{dir_name}/{idx:05d}.png')

    for idx, (img, _) in enumerate(tqdm(test_dataset)): 
        save_image(img, f'{data_dir}/test/{idx:05d}.png')
